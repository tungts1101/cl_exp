...
class ReplayBuffer(Dataset):
    ...
    def update(self):
        self._xs = []
        self._ys = []
        for clz in self._classes:
            if len(self._classes[clz]) > self._buffer_size:
                random.shuffle(self._classes[clz])
                self._classes[clz] = self._classes[clz][:self._buffer_size]
            self._xs.extend(self._classes[clz])
            self._ys.extend([clz] * len(self._classes[clz]))
    
    def add(self, x, y):
        if y not in self._classes:
            self._classes[y] = []
        self._classes[y].append(x)
    ...
    def finetune(self, train_loader):
        ...
        for _, epoch in enumerate(pbar):
            ...
            for i, (_, _, x, y) in enumerate(train_loader):
                ...
                info = f"Epoch {epoch}, Loss: {total_loss / total:.4f}, Acc: {correct * 100 / total:.2f}"
        ...
        for (_, _, x, y) in train_loader:
            for i in range(len(x)):
                self._buffer.add(x[i], y[i].cpu().item())
        self._buffer.update()
        # for clz in self._buffer._classes.keys():
        #     print(f"Class {clz}: {len(self._buffer._classes[clz])}")
        
        # batch_size = 16 if len(self._buffer) < 2000 else 48
        ...
        for task in range(self.cur_task + 1):
            ...
            epochs = 1
            min_lr = 0.0
            optimizer = optim.AdamW(task_model.head.parameters(), lr=3e-5, weight_decay=1e-2)
            scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs, eta_min=min_lr)
            ...
            for _, epoch in enumerate(pbar):
                ...
                for i, (x, y) in enumerate(loader):
                    ...
                    en_loss = (F.relu(E_in - m_in) ** 2).mean() + (F.relu(m_out - E_out) ** 2).mean()
                    ...
                    info = f"Task {task}, Epoch {epoch}, Loss: {total_loss / total:.4f}, Acc: {correct * 100 / total:.2f}"
                    pbar.set_description(info)
