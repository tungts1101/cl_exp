...
def setup_logger(log_file=f'logs/{timestamp}_exp48.log'):
...
class WelfordNormBatch(nn.Module):
    ...
    def __repr__(self):
        return f"WelfordNorm(count={self.count}, mean={self.mean.mean()}, m2={self.m2.mean()})"
...
class WelfordAccumulator:
    def __init__(self, feature_dim):
        ...
        self.m2 = torch.zeros(feature_dim)

    def update(self, x):  # x: [batch_size, feature_dim]
        for xi in x:
            self.count += 1
            delta = xi - self.mean
    ...
    def get_mean_var(self, eps=1e-5):
        var = self.m2 / self.count
        return self.mean, var


def compute_mahalanobis_matrix(task_stats, eps=1e-5):
    task_ids = list(task_stats.keys())
    T = len(task_ids)
    dist_matrix = torch.zeros(T, T)

    for i in range(T):
        mu_i = task_stats[i]["mean"]
        var_i = task_stats[i]["var"]
        for j in range(T):
            if i == j:
                dist_matrix[i, j] = 0.0
            else:
                mu_j = task_stats[j]["mean"]
                var_j = task_stats[j]["var"]

                pooled_var = 0.5 * (var_i + var_j)
                diff = mu_i - mu_j
                dist = torch.sqrt((diff**2 / (pooled_var + eps)).sum())
...
def merge(base_params, tasks_params, coeff, method="ties", lamb=1.0, topk=100):
    params = {}
    for name in base_params:
        ...
        # tvs = [tv[i] * coeff[i] for i, tv in enumerate(tvs)]
...
class Learner:
    def __init__(self, config):
        ...
        self._session_stats = {}
        self._session_to_class_ids = {}
        self._class_prototypes = {}
        self._session_accumulator = WelfordAccumulator(self.model.feature_dim)
        self._session_prototypes = {}
        self._session_coeff = []
    ...
    def merge(self, session_coeff=None):
        if self._config["model_merge_base"] == "zero":
            state_dict = torch.load(self.backbone_base())
            base_params = {k: torch.zeros_like(v).cpu() for k, v in state_dict.items()}
        elif self._config["model_merge_base"] == "init":
        ...
        if session_coeff == None:
            session_coeff = [1.0] * len(session_params)
        backbone_params = merge(base_params, session_params, session_coeff, method=self._config["model_merge"], lamb=self._config["model_merge_coef"], topk=self._config["model_merge_topk"])
    ...
    def train(self):
        ...
        # if self._config["train_reweight_method"] == "merge":
        #     self.merge()
        #     embedding_list = []
        #     label_list = []
        #     with torch.no_grad():
        ...
        #     class_list=np.unique(trainset.labels)
        #     self._session_to_class_ids[self._cur_session] = class_list
        #     for class_index in class_list:
        #         data_index=(label_list==class_index).nonzero().squeeze(-1)
        #         embedding=embedding_list[data_index]
        #         proto=embedding.mean(0)
        #         self.model.fc.weight.data[class_index]=proto
        #         self._class_prototypes[class_index] = proto
        # elif self._config["train_reweight_method"] == "none":
        #     self.model.backbone.load_state_dict(session_model.backbone.state_dict(), strict=True)
        #     embedding_list = []
        #     label_list = []
        #     with torch.no_grad():
        #         for i, batch in tqdm(enumerate(train_loader)):
        ...
        #         self.model.fc.weight.data[class_index]=proto
        #         self._class_prototypes[class_index] = proto
        # elif self._config["train_reweight_method"] == "first_session":
        ...
        #         proto=embedding.mean(0)
        #         self.model.fc.weight.data[class_index] = proto
        # elif self._config["train_reweight_method"] == "mahalanobis":        
        #     self.merge()
        #     # Mahalanobis distance
        #     embedding_list = []
        #     label_list = []
        #     with torch.no_grad():
        #         task_accumulator = WelfordAccumulator(self.model.feature_dim)
        #         for i, batch in tqdm(enumerate(train_loader)):
        #             (_, _, data,label)=batch
        #             data=data.cuda()
        #             label=label.cuda()
        ...
        #             embedding_list.append(embedding.cpu())
        #             label_list.append(label.cpu())
        #             task_accumulator.update(embedding.cpu())
                    
        #     mean, var = task_accumulator.get_mean_var()
        #     self._session_stats[self._cur_session] = {
        #         "mean": mean,
        #         "var": var
        #     }
        #     dist_matrix = compute_mahalanobis_matrix(self._session_stats)
        ...
        #     gamma = 1.0
        #     for session_id in range(self._cur_session):
        #         dist = dist_matrix[session_id, self._cur_session].item()
        #         beta = torch.exp(-gamma * torch.tensor(dist))
        #         class_ids = self._session_to_class_ids[session_id]
        #         print(f"Session {session_id + 1}, Beta: {beta.item()}")
        #         for cid in class_ids:
        #             self.model.fc.weight.data[cid] = self._class_prototypes[cid] * (1 - beta)
        # elif self._config["train_reweight_method"] == "cosine":
        #     self.merge()
        #     # Cosine similarity
        #     backbone = get_backbone(self._config)
        #     backbone.cuda()
        #     backbone.eval()
        #     old_embeddings = [[] for _ in range(self._cur_session)]
        #     embedding_list = []
        #     label_list = []
        #     with torch.no_grad():
        #         for i, batch in tqdm(enumerate(train_loader)):
        ...
        #             for session_id in range(self._cur_session):
        #                 backbone.load_state_dict(torch.load(backbone_checkpoint(session_id)), strict=False)
        #                 old_embeddings[session_id].append(backbone(x).cpu())
        ...
        #     old_embeddings = [torch.cat(embeddings, dim=0) for embeddings in old_embeddings]
        #     old_prototypes = [embedding.mean(0) for embedding in old_embeddings]
        #     new_prototype = embedding_list.mean(0)
        #     betas = [F.cosine_similarity(new_prototype, old_prototype, dim=0) for old_prototype in old_prototypes]
        #     print(f"Betas: {betas}")
            
        #     for session_id in range(self._cur_session):
        #         class_ids = self._session_to_class_ids[session_id]
        #         for cid in class_ids:
        #             self.model.fc.weight.data[cid] = self._class_prototypes[cid] * betas[session_id]
        # elif self._config["train_reweight_method"] == "first_session_merge":
        #     session_model.eval()
        #     backbone = get_backbone(self._config).cuda().eval()
        #     backbone.load_state_dict(torch.load(backbone_checkpoint(0)), strict=False)
            
        #     fs_embedding_list = [] # first session embedding
        #     cs_embedding_list = [] # current session embedding
        #     with torch.no_grad():
        #         for i, batch in tqdm(enumerate(train_loader)):
        #             (_, _, data,label)=batch
        #             data=data.cuda()
        #             label=label.cuda()
        #             fs_embedding = backbone(data)
        #             fs_embedding_list.append(fs_embedding.cpu())
        #             cs_embedding = session_model.get_features(data)
        #             cs_embedding_list.append(cs_embedding.cpu())
            
        #     fs_embedding_list = torch.cat(fs_embedding_list, dim=0).mean(0)
        #     cs_embedding_list = torch.cat(cs_embedding_list, dim=0).mean(0)
            
        #     self._session_coeff.append(F.cosine_similarity(fs_embedding_list, cs_embedding_list, dim=0))
        #     print(f"Session Coeff: {self._session_coeff}")
        #     self.merge(self._session_coeff)
        ...
        #         proto=embedding.mean(0)
        #         self.model.fc.weight.data[class_index] = proto
        # elif self._config["train_reweight_method"] == "cosine_merge":
        #     session_model.eval()
        #     backbone = get_backbone(self._config)
        #     backbone.cuda()
        #     backbone.eval()
        ...
        #             (_, _, x, y)=batch
        #             x, y = x.cuda(), y.cuda()
        #             embedding = session_model.get_features(x)
        #             now_embedding.append(embedding.cpu())

        #             for session_id in range(self._cur_session):
        #                 backbone.load_state_dict(torch.load(backbone_checkpoint(session_id)), strict=False)
        #                 old_embeddings[session_id].append(backbone(x).cpu())
            
        #     now_embedding = torch.cat(now_embedding, dim=0).mean(0)
        #     old_embeddings = [torch.cat(embeddings, dim=0).mean(0) for embeddings in old_embeddings]
        #     session_coeff = [F.cosine_similarity(now_embedding, old_embedding, dim=0) for old_embedding in old_embeddings]
        #     session_coeff.append(1.0) # coeff of current session
        #     print(f"Session Coeff: {session_coeff}")
        #     self.merge(session_coeff)
...
for model_backbone in ["vit_base_patch16_224_lora", "vit_base_patch16_224_in21k_lora"]:
    for dataset_name in ["cifar224", "imagenetr", "imageneta", "cub", "omnibenchmark", "vtab"]:
        ...
        for seed in [1993]:
            ...
            for model_merge in ["max", "ties"]:
                for model_merge_base in ["init", "first"]:
                    model_config = {
                        "model_backbone": model_backbone,
                        "model_merge": model_merge,
                        "model_merge_base": model_merge_base,
