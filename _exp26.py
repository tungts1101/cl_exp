...
def setup_logger(log_file=f"logs/{timestamp}_exp26.log"):
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.adapter = ScaleShift(self.backbone.num_features)
...
class Learner:
    ...
    def eval(self, test_loader):
        ...
        params, param_names = collect_params(self.model.adapter)
        # print(param_names)
        optimizer = optim.SGD(params, lr=1e-4)
        tented_model = Tent(self.model, optimizer, steps=10, episodic=True)

        # with torch.no_grad():
        for _, (_, x, y) in tqdm(enumerate(test_loader)):
            x, y = x.cuda(), y.cuda()
            
            # print(self.model.adapter.scale[:5])
