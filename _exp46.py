...
class Learner:
    ...
    def fit(self):
        if self._cur_task == 0:
            M = self._config["model_M"] if self._config["model_ranpac"] else self.model.feature_dim
        ...
        H = F.relu(fs @ self.model.fc.W_rand.cpu()) if self.model.fc.W_rand is not None else fs
