...
class Learner:
    ...
    def learn(self, data_manager):
        ...
        self.buffer = RandomReplayBuffer(config["train_buffer_size"], decay=0.9)
    ...
    def train(self):
        ...
        if self._cur_task > 0:
            epochs = 10
            optimizer = optim.SGD(self.model.backbone.parameters(), lr=1e-2, momentum=0.9, weight_decay=5e-4)
            ...
            for epoch in range(epochs):
                ...
                for _, batch in enumerate(self.buffer.__iter__(batch_size=64)):
                    x, z, y = batch
                ...
                logger.info(
                    ...
                    f"Total Loss: {log['total_loss']:.4f}"
                )
        
        # self.model.update_head(self._total_classes - self._known_classes, freeze_old=True)
        ...
        #         log = {
        #             "entropy_loss": total_loss / total_samples
        #         }

        #         logger.info(
        #             f"Task {self._cur_task + 1}, Epoch {epoch + 1}/{epochs}, "
        #             f"Entropy Loss: {log['entropy_loss']:.4f}"
        #         )
    ...
    def prefix(self):
        return f"{self._config['seed']}_{self._config['dataset_name']}_{self._config['dataset_num_task']}_{self._config['model_backbone']}_{self._config['train_method']}_align"
