...
class Model(nn.Module):
    def __init__(self):
        ...
        self.program = Program(mask_size=224)
            ...
            for _, (_, x, y) in tqdm(enumerate(test_loader)):
                ...
                for _, (ix, iy) in enumerate(zip(x, y)):
                    ...
                    for i, program in enumerate(self.programs):
                        ...
                        mean_entropies.append(entropy.squeeze(0)[i*20:(i+1)*20].mean().item())
                        sum_entropies.append(entropy.squeeze(0)[i*20:(i+1)*20].sum().item())
                        min_entropies.append(entropy.squeeze(0)[i*20:(i+1)*20].min().item())
                        max_entropies.append(entropy.squeeze(0)[i*20:(i+1)*20].max().item())
                        
                        total_mean_entropies.append(entropy.squeeze(0).mean().item())
                        total_sum_entropies.append(entropy.squeeze(0).sum().item())
                        total_min_entropies.append(entropy.squeeze(0).min().item())
                        
                    print(f"Mean entropies: {mean_entropies}, Sum entropies: {sum_entropies}, Min entropies: {min_entropies}, Max entropies: {max_entropies}")
                    print(f"Total Mean entropies: {total_mean_entropies}, Total Sum entropies: {total_sum_entropies}, Total Min entropies: {total_min_entropies}")
    ...
    def finetune(self, train_loader):
        ...
        for _, epoch in enumerate(pbar):
            ...
            for i, (_, x, y) in enumerate(train_loader):
                ...
                info = f"Epoch {epoch}, Samples: {total * 100 / len(train_loader.dataset):.2f}, Loss: {total_loss / total:.4f}, Cls loss: {total_cls_loss / total:.4f}, Acc: {correct * 100 / total:.2f}"
