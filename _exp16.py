...
class Model(nn.Module):
    def __init__(self):
        ...
        self.prompter = PadPrompt((3, 224, 224), 20)
...
class Learner:
        ...
        with torch.no_grad():
            for _, (_, x, y) in tqdm(enumerate(test_loader)):
                ...
                for prompt in self.prompt_pool:
                    self.model.prompter.load_state_dict(prompt)
                    logits = self.model(x)
                    values, predicts = torch.topk(logits, k=1, dim=1, largest=True, sorted=True)
                    candidates.append((values.squeeze(), predicts.squeeze()))
                    
                candidate_values = torch.stack([cand[0] for cand in candidates], dim=1)
                candidate_predictions = torch.stack([cand[1] for cand in candidates], dim=1)
                top_value, top_value_idx = torch.max(candidate_values, dim=1)
                top_predicted_classes = torch.gather(candidate_predictions, dim=1, index=top_value_idx.unsqueeze(1))
                
                y_pred.append(top_predicted_classes.cpu().numpy())
    ...
    def finetune(self, train_loader, test_loader, task, epochs=20):
        ...
        self.prompt_pool.append(task_model.prompter.state_dict())
