...
def merge_max_abs(task_vectors):
    ...
    with torch.no_grad():
        ...
        for key in task_vectors[0].vector:
            ...
            max_abs_tensor = copy.deepcopy(task_vectors[0].vector[key])
...
def merge_avg(task_vectors):
    """Mix multiple task vectors together by averaging."""
    with torch.no_grad():
        new_vector = {}
        for key in task_vectors[0].vector:
            new_vector[key] = sum([task_vector.vector[key] for task_vector in task_vectors]) / len(task_vectors)
