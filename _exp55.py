...
def setup_logger(log_file=f'logs/{timestamp}_exp55.log'):
...
class CouplingLayer(nn.Module):
    """Affine coupling layer for conditional flows."""
    def __init__(self, feature_dim, condition_dim, hidden_dim, mask):
        super().__init__()
        ...
        self.scale_net     = nn.Linear(hidden_dim, feature_dim, bias=True)
        self.translate_net = nn.Linear(hidden_dim, feature_dim, bias=True)
        nn.init.zeros_(self.scale_net.weight);     nn.init.zeros_(self.scale_net.bias)
        nn.init.zeros_(self.translate_net.weight); nn.init.zeros_(self.translate_net.bias)

    def forward(self, x, condition, reverse=False):
        x_a = x * self.mask                                         # masked part
        h   = self.net(torch.cat([x_a, condition], dim=1))
        s   = torch.tanh(self.scale_net(h)) * (1 - self.mask)       # only on un-masked dims
        t   = self.translate_net(h)      * (1 - self.mask)
...
class ActNorm(nn.Module):
    """Activation Normalization (data-dependent init)."""
    def __init__(self, num_features):
        super().__init__()
        self.register_parameter("scale", nn.Parameter(torch.ones(num_features)))
    ...
    def forward(self, x, reverse=False):
        if (self.training) and (not self.initialized):
            with torch.no_grad():
                mean = x.mean(0); std = x.std(0).clamp_min_(1e-6)
                self.bias.data  = -mean
        ...
        if not reverse:
            ...
            log_det = self.scale.abs().log().sum().expand(x.size(0))
        else:
...
class ConditionalNormalizingFlow(nn.Module):
    def __init__(self, cfg: Config):
        super().__init__()
        self.config        = cfg
        self.feature_dim   = cfg.generative_feature_dim
        self.condition_dim = cfg.total_num_classes
        ...
        for i in range(num_layers):
            ...
            mask[i % 2 :: 2] = 1     # even / odd pattern
            masks.append(mask)

        layers = []
        for i in range(num_layers):
            layers.append(ActNorm(self.feature_dim))
        ...
        self.final_actnorm = ActNorm(self.feature_dim)

    # ── NEW: batch-wise sampling interface ──────────────────────
    @torch.no_grad()
    def sample(self, labels: torch.Tensor):
        """
        labels : LongTensor (B,) of class-ids.
        returns : features (B, feature_dim)
        """
        z   = torch.randn(labels.size(0), self.feature_dim, device=labels.device)
        ctx = F.one_hot(labels, num_classes=self.condition_dim).float()
        x, _ = self.forward(z, ctx, reverse=True)
        return x

    # single-value convenience (kept for backward compat)
    def sample_one(self, n, c):
        lab = torch.full((n,), c, dtype=torch.long, device=self.config.device)
        return self.sample(lab)

    # -----------------------------------------------------------
    def forward(self, x, condition, reverse=False):
        ld_total = torch.zeros(x.size(0), device=x.device)
        ...
        if not reverse:
            ...
            for layer in self.layers:
                if isinstance(layer, ActNorm):
                    ...
                    z, ld = layer(z, condition, reverse=False)
                ld_total += ld
            z, ld = self.final_actnorm(z, reverse=False)
            ld_total += ld
            return z, ld_total
        else:
            x_recon = x
            x_recon, ld = self.final_actnorm(x_recon, reverse=True)
            ld_total += ld
            for layer in reversed(self.layers):
                if isinstance(layer, ActNorm):
                    x_recon, ld = layer(x_recon, reverse=True)
                else:
                    x_recon, ld = layer(x_recon, condition, reverse=True)
                ld_total += ld
            return x_recon, ld_total

    # -----------------------------------------------------------
    def log_prob(self, x, condition):
        z, ld = self.forward(x, condition, reverse=False)
        log_pz = -0.5 * (z.pow(2) + math.log(2 * math.pi)).sum(1)
        return log_pz + ld


# ────────────────────────────────────────────────────────────────
def flow_nll(model, x, cond):
    return -model.log_prob(x, cond).mean()


@torch.no_grad()
def estimate_stats(backbone: nn.Module, loader: DataLoader, device):
    s1, s2, n = 0.0, 0.0, 0
    backbone.eval()
    for _, imgs, _ in loader:
        ...
        s2 += (f ** 2).sum(0)
    mean = s1 / n
    std  = (s2 / n - mean ** 2).sqrt().clamp_min_(1e-6)
    return mean, std


def ema_actnorm(cur_model, prev_model, beta=0.995):
    """EMA only for ActNorm parameters."""
    for m_cur, m_prev in zip(cur_model.modules(), prev_model.modules()):
        if isinstance(m_cur, ActNorm):
            m_cur.scale.data.mul_(beta).add_((1 - beta) * m_prev.scale.data)
            m_cur.bias .data.mul_(beta).add_((1 - beta) * m_prev.bias .data)
...
class Learner:
    ...
    def learn(self, data_manager):
        ...
        self.generator = ConditionalNormalizingFlow(cfg).to(self.device)
    ...
    def merge(self):
        ...
        session_params = [torch.load(self.backbone_checkpoint(session)) for session in range(self._cur_task + 1)]
    ...
    def train(self):
        ...
        if not os.path.exists(self.backbone_checkpoint(self._cur_task)) or not os.path.exists(self.head_checkpoint(self._cur_task)) or self._config["reset"]:
            ...
            if self._config["train_method"] == "seq": # sequential training
                ...
                if self._cur_task > 0:
                    task_model.backbone.load_state_dict(torch.load(self.backbone_checkpoint(0)), strict=False)
        ...
        for epoch in range(epochs):
            ...
            for i, (x, z, y) in enumerate(self.buffer, B):
                ...
                # z_scatter.scatter_(1, y.unsqueeze(1), z)
...
for model_backbone in ["vit_base_patch16_224_in21k_lora"]:
