...
from inc_net import ResNetCosineIncrementalNet,SimpleVitNet, ESNNet
    ...
    def __init__(self, args):
        ...
        if args["model_name"]!='ncm':
            ...
            if 'resnet' in args['convnet_type']:
                ...
                self._network = ESNNet(args, True)
    ...
    def replace_fc(self,trainloader):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(trainloader)):
                ...
                self._network.get_conv_features(data)
                # f = self._network.features
                # embedding = f[-1][:, 0, :]
                # embedding = self._network.features
                
                h = self._network.get_esn_features(self._network.features)
                Features_h.append(h.cpu())
                label_list.append(label.cpu())
                
        Features_h = torch.cat(Features_h, dim=0)
        label_list = torch.cat(label_list, dim=0)
        Y=F.one_hot(label_list,self.total_classnum).float()
