...
class Learner:
    ...
    def finetune(self, train_loader, test_loader, task, epochs=20):
        ...
        #         info = f"Epoch {epoch}, Samples: {total * 100 / len(train_loader.dataset):.2f}, Total loss: {total_loss / total:.4f}, Acc: {correct * 100 / total:.2f}"
        ...
        for _, _, y in train_loader:
            learned_classes.update(y.numpy())
        
        for clz in learned_classes:
            self.buffer.add(torch.randn(256, 3, 224, 224), (torch.ones(256) * clz).long())
        
        buffer_loader = DataLoader(self.buffer, batch_size=64, shuffle=True)
        self.model.eval()
        pbar = tqdm(range(10))
        for epoch in pbar:
            total, total_loss, correct = 0, 0, 0
            for i, (x, y) in enumerate(buffer_loader):
                x, y = x.cuda(), y.cuda()
                ...
                pbar.set_description(f"Samples: {total * 100 / len(self.buffer):.2f}, Loss: {total_loss / total:.4f}, Acc: {correct * 100 / total:.2f}")
