...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        if tuning_mode == "dyt":
            self.dyt_1 = DyT(hidden_features, 1.0)
            self.dyt_2 = DyT(out_features, 1.0)

    def forward(self, x):
        x = self.fc1(x)
        if self.tuning_mode == "dyt":
            x = self.dyt_1(x)
...
class Attention(nn.Module):
    def __init__(
        ...
        self.tuning_mode = tuning_mode
        if tuning_mode == "dyt":
            self.dyt_1 = DyT(dim * 3, 1.0)
            self.dyt_2 = DyT(dim, 1.0)

    def forward(self, x):
        B, N, C = x.shape
        if self.tuning_mode == "dyt":
            qkv = (
                (self.dyt_1(self.qkv(x)))
    ...
    def forward(self, x):
        if self.tuning_mode == "dyt":
            x = x + self.drop_path1(
                self.ls1(
                    self.attn(
                        self.dyt_1(self.norm1(x))
                    )
                )
            )
            x = x + self.drop_path2(
                self.ls2(
                    self.mlp(self.dyt_2(self.norm2(x)))
                )
            )
        else:
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.tuning_mode = tuning_mode
        if tuning_mode == "dyt":
            self.dyt_1 = DyT(embed_dim, 1.0)

            if norm_layer:
                self.dyt_2 = DyT(embed_dim, 1.0)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if tuning_mode == "dyt":
            self.dyt_1 = DyT(self.num_features, 1.0)
...
@register_model
def vit_tiny_patch16_224_dyt(pretrained=False, **kwargs):
    """ViT-Tiny (Vit-Ti/16)"""
...
@register_model
def vit_tiny_patch16_384_dyt(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch16_224_dyt(pretrained=False, **kwargs):
    """ViT-Small (ViT-S/16)
...
@register_model
def vit_small_patch16_384_dyt(pretrained=False, **kwargs):
    """ViT-Small (ViT-S/16)
    NOTE I've replaced my previous 'small' model definition and weights with the small variant from the DeiT paper
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
...
@register_model
def vit_base_patch16_224_dyt(pretrained=False, **kwargs):
...
@register_model
def vit_base_patch16_384_dyt(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch16_224_dyt(pretrained=False, **kwargs):
...
@register_model
def vit_large_patch16_384_dyt(pretrained=False, **kwargs):
    """ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
...
@register_model
def vit_tiny_patch16_224_in21k_dyt(pretrained=False, **kwargs):
    """ViT-Tiny (Vit-Ti/16).
...
@register_model
def vit_small_patch16_224_in21k_dyt(pretrained=False, **kwargs):
    """ViT-Small (ViT-S/16)
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_base_patch16_224_in21k_dyt(pretrained=False, **kwargs):
    """ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
...
@register_model
def vit_large_patch16_224_in21k_dyt(pretrained=False, **kwargs):
    """ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
