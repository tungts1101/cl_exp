    ...
    def freeze_backbone(self):
        ...
        for name, param in self.base.named_parameters():
            if (
                ...
                self.ewc_params.append((name, param))
                self.param_star[name] = param.data.clone().cuda()
                self.fisher[name] = torch.zeros_like(param).cuda()

    def compute_fisher(self, trainloader):
        self.eval()
        for name, param in self.ewc_params:
            self.fisher[name] = torch.zeros_like(param).cuda()
        ...
        for i, (_, x, y) in enumerate(trainloader):
            ...
            with torch.no_grad():
                for name, param in self.ewc_params:
                    ...
                    self.param_star[name] = param.data.clone()

        for name, param in self.ewc_params:
            self.fisher[name] /= len(trainloader)
            # print(f"Fisher {name}: {self.fisher[name].mean()}")
    ...
    def tune(self, trainloader, testloader, starting_label):
        ...
        for _, epoch in enumerate(pbar):
            ...
            for i, (_, x, y) in enumerate(trainloader):
                ...
                task_loss = F.cross_entropy(F.softmax(logits, dim=1), y)

                ewc_loss = 0
                for name, param in self.model.ewc_params:
                    fisher = self.model.fisher[name]
                    param_star = self.model.param_star[name]
                    # print(f"Fisher {name}: {fisher.mean()}")
                    ewc_loss += (fisher * (param - param_star) ** 2).sum()
                    # print(f"EWC loss {name}: {ewc_loss}")
                ewc_loss = ewc_loss * 5
                ...
                # print(f"Task loss: {task_loss}, EWC loss: {ewc_loss}")
            ...
            info = "Loss {:.3f}, Train_acc {:.2f}, Test_acc {:.2f}, EWC loss {:.2f}".format(
        ...
        self.model.compute_fisher(trainloader)
