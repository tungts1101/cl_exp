...
class Learner:
    ...
    def finetune(self, train_loader, test_loader, epochs=20):
        ...
        params = [
            {"params": self.model.backbone.parameters(), "lr": body_lr, "momentum": 0.9, "weight_decay": weight_decay},
            {"params": self.model.head.parameters(), "lr": body_lr, "momentum": 0.9, "weight_decay": weight_decay}
        ]
        
        optimizer = optim.SGD(params, lr=body_lr, momentum=0.9, weight_decay=weight_decay)
        ...
        for _, epoch in enumerate(pbar):
            ...
            for i, (_, x, y) in enumerate(train_loader):
                x, y = x.cuda(), y.cuda()
                y = torch.where(y < self._known_classes, y, -100)
                logits = self.model(x)
        ...
        #     if "ssf_" in name:
        #         merged_tv = operator.attrgetter(name)(self.model.backbone).detach().clone()
        #         tv = operator.attrgetter(name)(task_model.backbone).detach().clone()
        #         merged_tv = lambd * tv + (1 - lambd) * merged_tv
                
        #         operator.attrgetter(name)(self.model.backbone).data.copy_(merged_tv)
        ...
        # print(self.model.head.heads[-1])
        # self.model.head.heads[-1][-1].weight.data.copy_(task_model.head.heads[-1][-1].weight.data)
